...
describe('Element', function() {
  ...
  describe('#addAnimation()', function() {
    it('should create a new attribute if one doesn\'t exist', function() {
      var testAnimation = { attribute: 'test', start: 0, duration: 0, destination: 0, ease: testEasingFunction, delay: 0, loop: false, wait: 0 };
    ...
    it('should add a new animation to an attribute if it already exists', function() {
      var testAnimation1 = { attribute: 'test', start: 0, duration: 0, destination: 0, ease: testEasingFunction, delay: 0, loop: false, wait: 0 };
      var testAnimation2 = { attribute: 'test', start: 1, duration: 0, destination: 0, ease: testEasingFunction, delay: 0, loop: false, wait: 0 };
    ...
    it('should add an animation with the correct parameters', function() {
      var testAnimation = { attribute: 'test', start: 0, duration: 60, destination: 20, ease: testEasingFunction, delay: 30, loop: true, wait: 25 };
      testElement.addAnimation(testAnimation);
      testElement.attributeMap.get('test').animations[0].should.eql({currentIteration: -30, startValue: -20, changeInValue: 20, totalIterations: 60, easingFunction: testEasingFunction, loop: true, delay: 30, wait: 25 });
    });
  });
});