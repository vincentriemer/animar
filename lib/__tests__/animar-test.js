...
describe('_addAnimation()', function() {
  ...
  it('should correctly set default values to missing options object', function() {
    animar._addAnimation(testElement, 'translateX', 10);
    expect(animar._addAnimationToMap.mock.calls[0][0]).toEqual({
      element: testElement,
      attribute: 'translateX',
  ...
  it('should correctly handle the elastic easing function as a string name', function() {
    require('../ease').elastic = jest.genMockFn().mockReturnValue('expected result');
    animar._addAnimation(testElement, 'translateX', 10, { easing: 'elastic', damping: 0.3 });
    expect(require('../ease').elastic.mock.calls.length).toEqual(1);
    expect(require('../ease').elastic.mock.calls[0][0]).toEqual(0.3);
    expect(animar._addAnimationToMap.mock.calls[0][0].ease).toEqual('expected result');
  });
